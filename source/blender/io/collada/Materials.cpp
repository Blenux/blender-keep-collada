...
MaterialNode::MaterialNode(blender::bContext *C, blender::Material *ma, KeyImageMap &key_image_map)
    : mContext(C), material(ma), effect(nullptr), key_image_map(&key_image_map)
{
  blender::bNodeTree *new_ntree = prepare_material_nodetree();
  setShaderType();
  if (new_ntree) {
...
MaterialNode::MaterialNode(blender::bContext *C,
    ...
    : mContext(C), material(ma), effect(ef), uid_image_map(&uid_image_map)
{
  ...
  std::map<std::string, blender::bNode *> nmap;
#if 0
...
blender::bNodeTree *MaterialNode::prepare_material_nodetree()
{
...
blender::bNode *MaterialNode::add_node(int node_type, int locx, int locy, std::string label)
{
  blender::bNode *node = blender::bke::node_add_static_node(mContext, *ntree, node_type);
  if (node) {
    if (label.length() > 0) {
      blender::STRNCPY(node->label, label.c_str());
    }
    node->location[0] = locx;
    node->location[1] = locy;
    node->flag |= blender::NODE_SELECT;
  }
  node_map[label] = node;
  return node;
}

void MaterialNode::add_link(blender::bNode *from_node, int from_index, blender::bNode *to_node, int to_index)
{
  blender::bNodeSocket *from_socket = (blender::bNodeSocket *)BLI_findlink(&from_node->outputs, from_index);
  blender::bNodeSocket *to_socket = (blender::bNodeSocket *)BLI_findlink(&to_node->inputs, to_index);

  blender::bke::node_add_link(*ntree, *from_node, *from_socket, *to_node, *to_socket);
}

void MaterialNode::add_link(blender::bNode *from_node,
                            const char *from_label,
                            blender::bNode *to_node,
                            const char *to_label)
{
  blender::bNodeSocket *from_socket = blender::bke::node_find_socket(*from_node, blender::SOCK_OUT, from_label);
  blender::bNodeSocket *to_socket = blender::bke::node_find_socket(*to_node, blender::SOCK_IN, to_label);
...
void MaterialNode::set_reflectivity(COLLADAFW::FloatOrParam &val)
{
  ...
  if (reflectivity >= 0) {
    blender::bNodeSocket *socket = blender::bke::node_find_socket(*shader_node, blender::SOCK_IN, "Metallic");
    ((blender::bNodeSocketValueFloat *)socket->default_value)->value = reflectivity;
...
void MaterialNode::set_ior(COLLADAFW::FloatOrParam &val)
{
  ...
  blender::bNodeSocket *socket = blender::bke::node_find_socket(*shader_node, blender::SOCK_IN, "IOR");
  ((blender::bNodeSocketValueFloat *)socket->default_value)->value = ior;
}
                             ...
                             COLLADAFW::FloatOrParam &val)
{
  ...
  if (cot.isColor() || !cot.isValid()) {
    ...
    blender::bNodeSocket *socket = blender::bke::node_find_socket(*shader_node, blender::SOCK_IN, "Alpha");
    ((blender::bNodeSocketValueFloat *)socket->default_value)->value = alpha;
    material->a = alpha;
  }
...
void MaterialNode::set_diffuse(COLLADAFW::ColorOrTexture &cot)
{
  ...
  if (cot.isTexture()) {
    blender::bNode *texture_node = add_texture_node(cot, -300, locy, "Base Color");
    if (texture_node != nullptr) {
      add_link(texture_node, 0, shader_node, 0);
    }
  }
  else {
    blender::bNodeSocket *socket = blender::bke::node_find_socket(*shader_node, blender::SOCK_IN, "Base Color");
...
blender::Image *MaterialNode::get_diffuse_image()
{
  ntree->ensure_topology_cache();
  const blender::Span<const blender::bNode *> nodes = ntree->nodes_by_type("ShaderNodeBsdfPrincipled");
  if (nodes.is_empty()) {
    return nullptr;
  }
  const blender::bNode *shader = nodes.first();

  const blender::bNodeSocket *in_socket = blender::bke::node_find_socket(*shader, blender::SOCK_IN, "Base Color");
  if (in_socket == nullptr) {
    return nullptr;
  }

  const blender::bNodeLink *link = in_socket->link;
  if (link == nullptr) {
    return nullptr;
  }

  const blender::bNode *texture = link->fromnode;
  if (texture == nullptr) {
    return nullptr;
  }
  ...
  blender::Image *image = (blender::Image *)texture->id;
  return image;
}

static blender::bNodeSocket *set_color(blender::bNode *node, COLLADAFW::Color col)
{
  blender::bNodeSocket *socket = (blender::bNodeSocket *)BLI_findlink(&node->outputs, 0);
  float *fcol = (float *)socket->default_value;
  fcol[0] = col.getRed();
  fcol[1] = col.getGreen();
  fcol[2] = col.getBlue();
...
void MaterialNode::set_ambient(COLLADAFW::ColorOrTexture &cot)
{
  ...
  if (cot.isColor()) {
    COLLADAFW::Color col = cot.getColor();
    blender::bNode *node = add_node(SH_NODE_RGB, -300, locy, "Ambient");
    set_color(node, col);
    /* TODO: Connect node */
  }
  /* texture */
...
void MaterialNode::set_reflective(COLLADAFW::ColorOrTexture &cot)
{
  ...
  if (cot.isColor()) {
    COLLADAFW::Color col = cot.getColor();
    blender::bNode *node = add_node(SH_NODE_RGB, -300, locy, "Reflective");
    set_color(node, col);
    /* TODO: Connect node */
  }
  /* texture */
...
void MaterialNode::set_emission(COLLADAFW::ColorOrTexture &cot)
{
  ...
  if (cot.isColor()) {
    COLLADAFW::Color col = cot.getColor();
    blender::bNodeSocket *socket = blender::bke::node_find_socket(*shader_node, blender::SOCK_IN, "Emission Color");
    float *fcol = (float *)socket->default_value;
  ...
  // texture
    ...
    blender::bNode *texture_node = add_texture_node(cot, -300, locy, "Emission Color");
    if (texture_node != nullptr) {
      add_link(texture_node, "Color", shader_node, "Emission Color");
    }
  }

  blender::bNodeSocket *socket = blender::bke::node_find_socket(*shader_node, blender::SOCK_IN, "Emission Strength");
  if (socket) {
...
void MaterialNode::set_specular(COLLADAFW::ColorOrTexture &cot)
{
  ...
  if (cot.isColor()) {
    ...
    if (col.getRed() == 0 && col.getGreen() == 0 && col.getBlue() == 0) {
      ...
      blender::bNode *node = add_node(SH_NODE_RGB, -300, locy, "Specular IOR Level");
      set_color(node, col);
      /* TODO: Connect node */
    }
  }
  ...
  if (!has_specularity) {
    ...
    blender::bNodeSocket *socket = blender::bke::node_find_socket(
        *shader_node, blender::SOCK_IN, "Specular IOR Level");
    ((blender::bNodeSocketValueFloat *)socket->default_value)->value = 0.0f;
  }
}

blender::bNode *MaterialNode::add_texture_node(COLLADAFW::ColorOrTexture &cot,
                                      int locx,
                                      int locy,
                                      std::string label)
{
  ...
  blender::Image *ima = image_map[ima_uid];
  blender::bNode *texture_node = add_node(SH_NODE_TEX_IMAGE, locx, locy, label);
