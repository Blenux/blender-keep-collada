                                  ...
                                  std::vector<std::string> &layer_labels)
{
  ...
  for (skin_it = skin_by_data_uid.begin(); skin_it != skin_by_data_uid.end(); skin_it++) {
    ...
    if (b->get_joint_inv_bind_matrix(joint_inv_bind_mat, node)) {
      ...
      blender::Object *ob_arm = skin->BKE_armature_from_object();
      if (ob_arm) {
        float invmat[4][4];
  ...
  switch (use_connect) {
    case 1:
      bone->flag |= blender::BONE_CONNECTED;
      break;
...
void ArmatureImporter::fix_leaf_bone_hierarchy(blender::bArmature *armature,
                                               blender::Bone *bone,
                                               bool fix_orientation)
{
  ...
  for (blender::Bone &child : bone->childbase) {
    fix_leaf_bone_hierarchy(armature, &child, fix_orientation);
  }
}

void ArmatureImporter::fix_leaf_bone(blender::bArmature *armature,
                                     EditBone *ebone,
                                     BoneExtended *be,
                                     bool fix_orientation)
{
...
void ArmatureImporter::fix_parent_connect(blender::bArmature *armature, blender::Bone *bone)
{
  ...
  if (bone->parent && bone->flag & blender::BONE_CONNECTED) {
...
void ArmatureImporter::connect_bone_chains(blender::bArmature *armature,
                                           blender::Bone *parentbone,
                                           int max_chain_length)
{
  ...
  if (dominant_child != nullptr) {
    ...
    if (pebone && !(cebone->flag & blender::BONE_CONNECTED)) {
      float vec[3];
...
#if 0
void ArmatureImporter::set_leaf_bone_shapes(blender::Object *ob_arm)
{
...
{
  /* just set rotmode = blender::ROT_MODE_EUL on pose channel for each joint */
  ...
  for (it = joint_by_uid.begin(); it != joint_by_uid.end(); it++) {
    ...
    for (sit = skin_by_data_uid.begin(); sit != skin_by_data_uid.end(); sit++) {
      ...
      if (skin.uses_joint_or_descendant(joint)) {
        ...
        if (pchan) {
          pchan->rotmode = blender::ROT_MODE_EUL;
        }
        else {
...
blender::Object *ArmatureImporter::get_empty_for_leaves()
{
  if (empty) {
    return empty;
  }

  empty = bc_add_object(m_bmain, scene, view_layer, blender::OB_EMPTY, nullptr);
  empty->empty_drawtype = blender::OB_EMPTY_SPHERE;

  return empty;
}

#if 0
blender::Object *ArmatureImporter::find_armature(COLLADAFW::Node *node)
{
  ...
  for (int i = 0; i < children.getCount(); i++) {
    blender::Object *ob_arm = find_armature(children[i]);
    if (ob_arm) {
      return ob_arm;
    }
  }
...
ArmatureJoints &ArmatureImporter::get_armature_joints(blender::Object *ob_arm)
{
  /* try finding it */
...
#endif
void ArmatureImporter::create_armature_bones(blender::Main *bmain, std::vector<blender::Object *> &arm_objs)
{
  ...
  for (ri = root_joints.begin(); ri != root_joints.end(); ri++) {
    ...
    blender::Object *ob_arm = joint_parent_map[node->getUniqueId()];
    if (!ob_arm) {
      continue;
    }
    ...
    if (ob_arm->type != blender::OB_ARMATURE) {
      continue;
    }

    blender::bArmature *armature = (blender::bArmature *)ob_arm->data;
    if (!armature) {
      continue;
    }

    const char *bone_name = bc_get_joint_name(node);
    blender::Bone *bone = blender::BKE_armature_find_bone_name(armature, bone_name);
    if (bone) {
      fprintf(stderr,
...
blender::Object *ArmatureImporter::create_armature_bones(blender::Main *bmain, SkinInfo &skin)
{
  /* just do like so:
   * - get armature
   * - enter editmode
  ...
  ED_armature_to_edit(armature);
  if (this->import_settings->find_chains) {
    connect_bone_chains(armature, (blender::Bone *)armature->bonebase.first, UNLIMITED_CHAIN_MAX);
  }
  fix_leaf_bone_hierarchy(
      armature, (blender::Bone *)armature->bonebase.first, this->import_settings->fix_orientation);
  ED_armature_from_edit(bmain, armature);
  ED_armature_edit_free(armature);
...
void ArmatureImporter::set_bone_transformation_type(const COLLADAFW::Node *node, blender::Object *ob_arm)
{
  bPoseChannel *pchan = BKE_pose_channel_find_name(ob_arm->pose, bc_get_joint_name(node));
  if (pchan) {
    pchan->rotmode = node_is_decomposed(node) ? blender::ROT_MODE_EUL : blender::ROT_MODE_QUAT;
  }
...
void ArmatureImporter::set_pose(blender::Object *ob_arm,
                                ...
                                float parent_mat[4][4])
{
  ...
  pchan->rotmode = (is_decomposed) ? blender::ROT_MODE_EUL : blender::ROT_MODE_QUAT;
...
void ArmatureImporter::add_root_joint(COLLADAFW::Node *node, blender::Object *parent)
{
...
{
  // root_joints.push_back(node);
  blender::Object *ob_arm = find_armature(node);
  if (ob_arm) {
...
void ArmatureImporter::make_armatures(blender::bContext *C, std::vector<blender::Object *> &objects_to_scale)
{
  blender::Main *bmain = CTX_data_main(C);
  std::vector<blender::Object *> arm_objs;
  std::map<COLLADAFW::UniqueId, SkinInfo>::iterator it;
  ...
  for (it = skin_by_data_uid.begin(); it != skin_by_data_uid.end(); it++) {
    ...
    blender::Object *ob_arm = create_armature_bones(bmain, skin);

    /* link armature with a mesh object */
    const COLLADAFW::UniqueId &uid = skin.get_controller_uid();
    const COLLADAFW::UniqueId *guid = get_geometry_uid(uid);
    if (guid != nullptr) {
      blender::Object *ob = mesh_importer->get_object_by_geom_uid(*guid);
      if (ob) {
        skin.link_armature(C, ob, joint_by_uid, this);

        std::vector<blender::Object *>::iterator ob_it = std::find(
    ...
    blender::Object *par = skin.get_parent();
    if (par) {
  ...
  /* Fix bone relations */
  std::vector<blender::Object *>::iterator ob_arm_it;
  for (ob_arm_it = arm_objs.begin(); ob_arm_it != arm_objs.end(); ob_arm_it++) {

    blender::Object *ob_arm = *ob_arm_it;
    blender::bArmature *armature = (blender::bArmature *)ob_arm->data;

    /* and step back to edit mode to fix the leaf nodes */
    ED_armature_to_edit(armature);

    fix_parent_connect(armature, (blender::Bone *)armature->bonebase.first);
...
void ArmatureImporter::link_armature(blender::Object *ob_arm,
                                     const COLLADAFW::UniqueId &geom_id,
                                     const COLLADAFW::UniqueId &controller_data_id)
{
  blender::Object *ob = mesh_importer->get_object_by_geom_uid(geom_id);
...
void ArmatureImporter::make_shape_keys(blender::bContext *C)
{
  ...
  for (mc = morph_controllers.begin(); mc != morph_controllers.end(); mc++) {
    ...
    blender::Object *source_ob = this->mesh_importer->get_object_by_geom_uid((*mc)->getSource());

    if (source_ob) {

      blender::Mesh *source_me = (blender::Mesh *)source_ob->data;
      /* insert key to source mesh */
      blender::Key *key = source_me->key = blender::BKE_key_add(bmain, (blender::ID *)source_me);
      key->type = blender::KEY_RELATIVE;
      blender::KeyBlock *kb;

      /* insert basis key */
      kb = blender::BKE_keyblock_add_ctime(key, "Basis", false);
      blender::BKE_keyblock_convert_from_mesh(source_me, key, kb);
      ...
      for (int i = 0; i < morphTargetIds.getCount(); i++) {
        ...
        blender::Mesh *mesh = this->mesh_importer->get_mesh_by_geom_uid(morphTargetIds[i]);

        if (mesh) {
          mesh->key = key;
          std::string morph_name = *this->mesh_importer->get_geometry_name(mesh->id.name);

          kb = blender::BKE_keyblock_add_ctime(key, morph_name.c_str(), false);
          blender::BKE_keyblock_convert_from_mesh(mesh, key, kb);
...
blender::Object *ArmatureImporter::get_armature_for_joint(COLLADAFW::Node *node)
{
  std::map<COLLADAFW::UniqueId, SkinInfo>::iterator it;
  for (it = skin_by_data_uid.begin(); it != skin_by_data_uid.end(); it++) {
    SkinInfo &skin = it->second;
  ...
  std::map<COLLADAFW::UniqueId, blender::Object *>::iterator arm;
...
{
  char bone_name_esc[sizeof(blender::Bone::name) * 2];
  blender::BLI_str_escape(bone_name_esc, bc_get_joint_name(node), sizeof(bone_name_esc));
  blender::BLI_snprintf(joint_path, joint_path_maxncpy, "pose.bones[\"%s\"]", bone_name_esc);
}
...
BoneExtended &ArmatureImporter::add_bone_extended(blender::EditBone *bone,
                                                  COLLADAFW::Node *node,
                                                  int sibcount,
                                                  std::vector<std::string> &layer_labels,
